...
# -*- coding: utf-8 -*-
"""
5D BUDGETING — Módulo de presupuestación (repo 81) · servidor local STANDALONE
Estático + API mínima (opcional; la app funciona igual solo con el navegador):
  POST /api/save-project  -> guarda el proyecto JSON en proyectos/ (respaldo en disco)
  POST /api/delete-project-> borra el proyecto JSON de proyectos/
  GET  /api/email-status  -> indica si hay credenciales SMTP configuradas
  POST /api/send-email    -> envía correo vía SMTP (config.email.json), opcional
Sin dependencias externas: solo librería estándar. La persistencia real vive en
localStorage del navegador; este servidor solo añade un respaldo en disco.
"""
...
if __name__ == "__main__":
    ...
    with ServidorHilos(("::", PORT), Handler) as httpd:
        print(f"5D BUDGETING -> http://localhost:{PORT}/index.html")
        print(f"SMTP configurado: {'si' if leer_config_email() else 'no (opcional)'}")
        httpd.serve_forever()
